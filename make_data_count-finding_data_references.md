...
#### Kaggle情報のスクレイピング
- コンペティション概要、データセット説明、ディスカッションをスクレイピング（スクレイピングはサイトに高負荷をかけないようにリクエストとリクエストの間にsleepを入れないといけません）
- パブリックノートブックはAPIを利用してダウンロード
...
課金してハイスペックなGPUで高めのスペックのモデルを学習でもしないと、自分の実力ではメダルを取れないと判断しました。（取れませんでしたが😂）  
...
ディスク: 235.7 GB  
```
無料でもGPU（T4GPU 15GB GPUメモリ）は使えますので、LLMを試してみるであったり、ちょっとした利用に手軽でおすすめです。
